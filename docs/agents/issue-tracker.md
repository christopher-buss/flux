...
**PRs as a request surface: yes.** _(Set to `no` if this repo should stop
treating external PRs as feature requests; `/triage` reads this flag.)_

PRs run through the same labels and states as issues, using the `gh pr`
equivalents:
